...
void Silver::Camera::printCam() {
  ...
  int worldRangeMinX = std::min(silver.WorldRangeStart.x, silver.WorldRangeEnd.x);
  int worldRangeMaxX = std::max(silver.WorldRangeStart.x, silver.WorldRangeEnd.x);
  int worldRangeMinY = std::min(silver.WorldRangeStart.y, silver.WorldRangeEnd.y);
  int worldRangeMaxY = std::max(silver.WorldRangeStart.y, silver.WorldRangeEnd.y);
  ...
  if (markOutOfBounds) {
    for (int y = 0; y < silver.camera.CameraScale.y; ++y) {
        for (int x = 0; x < silver.camera.CameraScale.x; ++x) {
            ...
            int flippedX = (silver.camera.isFlippedHorizonal == -1) ? (silver.camera.CameraScale.x - x - 1) : x;
            int flippedY = (silver.camera.isFlippedVertical == -1) ? (silver.camera.CameraScale.y - y - 1) : y;

            // Convert flipped screen coordinates to world coordinates
            int worldX = round((flippedX - centerX) * cosTheta - (flippedY - centerY) * sinTheta) + silver.camera.CameraPos.x;
            int worldY = round((flippedX - centerX) * sinTheta + (flippedY - centerY) * cosTheta) + silver.camera.CameraPos.y;

            // Calculate actual world range boundaries
            
            // Mark out-of-bound areas
            if (worldX < worldRangeMinX || worldX > worldRangeMaxX || worldY < worldRangeMinY || worldY > worldRangeMaxY) {
                buffer[y][x] = out_rep; // Original (non-flipped) screen position is updated
            }
        }
    }
}
  ...
  for (const auto &entry : workspace) {
    ...
    if (obj.comp.isUI) {
       ...
       if (dx < worldRangeMinX || dx > worldRangeMaxX ||
    dy < worldRangeMinY || dy > worldRangeMaxY && cutOutOfBounds) {
...
void mesh::setTransparence(bool value) {
    isTransparent = value;
}
