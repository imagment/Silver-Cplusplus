...
Actor *GetActor(int objID) { return Workspace[objID]; }
...
void Actor::RemoveObject() {
  ...
  auto it = std::find(Workspace.begin(), Workspace.end(), target);
  if (it != Workspace.end()) {
    Workspace.erase(it);  // Erase the target object from the Workspace
  }

  if (id == nextObjectID - 1) --nextObjectID;
...
vector<Vector3> getRectangleHollowPoints(Vector3 topLeft, int width,
                                         int height) {
  vector<Vector3> points;
