...
void Silver::Scene::saveWorldAs(const string name) { scenes[name] = workspace; }

void Silver::Scene::changeSceneTo(const string name) {
...
void Silver::Camera::printCam() {
  ...
  int worldRangeMinX = min(silver.WorldRangeStart.x, silver.WorldRangeEnd.x);
  int worldRangeMaxX = max(silver.WorldRangeStart.x, silver.WorldRangeEnd.x);
  int worldRangeMinY = min(silver.WorldRangeStart.y, silver.WorldRangeEnd.y);
  int worldRangeMaxY = max(silver.WorldRangeStart.y, silver.WorldRangeEnd.y);
  ...
  if (markOutOfBounds) {
    for (int y = 0; y < silver.camera.CameraScale.y; ++y) {
      for (int x = 0; x < silver.camera.CameraScale.x; ++x) {
        ...
        int flippedX = (silver.camera.isFlippedHorizonal == -1)
                           ? (silver.camera.CameraScale.x - x - 1)
                           : x;
        int flippedY = (silver.camera.isFlippedVertical == -1)
  ...
  for (const auto &entry : workspace) {
    ...
    if (rotatedX >= 0 && rotatedX < silver.camera.CameraScale.x &&
      ...
      if (!printSpaces) {
        isTransparent = isTransparent || (obj.shape.find(' ') != string::npos ||
                                          obj.shape.find('\t') != string::npos);
      }
      if (!isTransparent) {
  ...
  for (const auto &line : leftTextLines) {
    maxLeftWidth = max(maxLeftWidth, static_cast<int>(line.size()));
  }
  for (const auto &line : rightTextLines) {
    maxRightWidth = max(maxRightWidth, static_cast<int>(line.size()));
  }
  ...
  if (!silver.camera.topText.empty()) {
    ...
    stringstream ssTop(topTextConcat);
    string line;
    while (getline(ssTop, line)) {
  ...
  if (!silver.camera.bottomText.empty()) {
    ...
    stringstream ssBottom(bottomTextConcat);
    string line;
    while (getline(ssBottom, line)) {
  ...
  for (int i = 0; i < topTextLines.size(); ++i) {
    ...
    if (silver.camera.topAlign == 1) {
      ...
      lineOffsetX +=
          (silver.camera.CameraScale.x * cellSize - topTextLines[i].size()) / 2;
    }
  ...
  if (!sideLimit) {
    ...
    renderedHeight =
        max(renderedHeight, max(leftTextLinesCount, rightTextLinesCount));
  }
  ...
  for (int j = 0; j < renderedHeight; ++j) {
    ...
    if (j < silver.camera.CameraScale.y) {
      ...
      for (int i = 0; i < silver.camera.CameraScale.x; ++i) {
        string cellContent = (!hideMouse && i == mouseX && j == mouseY)
                                 ? mouseIcon
                                 : buffer[j][i];
...
void Silver::Drawing::sprayOval(int spawns, Vec3 center, Vec3 scale, string c) {
  ...
  for (int i = 0; i < spawns; ++i) {
    ...
    Vec3 position = {center.x + static_cast<int>(distanceX * cos(angle)),
...
void Silver::Drawing::sprayRectangle(int spawns, Vec3 center, Vec3 scale,
                                     string c) {
  srand(static_cast<unsigned int>(time(nullptr)));
...
void Silver::Drawing::sprayLine(int spawns, Vec3 start, Vec3 end, string c) {
  ...
  for (int i = 0; i < spawns; ++i) {
    ...
    Vec3 position = {static_cast<int>(start.x + t * dx),
...
void Silver::Drawing::RectangleHollow(Vec3 topLeft, int width, int height,
                                      string c) {
...
void Silver::Drawing::OvalHollow(Vec3 center, int radiusX, int radiusY,
                                 string c) {
  for (int y = -radiusY; y <= radiusY; y++) {
    for (int x = -radiusX; x <= radiusX; x++) {
      int equationValue =
...
void Silver::Rectangle(const string name, int number, Vec3 topLeft, int width,
                       int height) {
  for (int i = 0; i < width; ++i) {
    for (int j = 0; j < height; ++j) {
      Vec3 pos = {topLeft.x + i, topLeft.y + j, topLeft.z};
...
void Silver::RectangleHollow(const string name, int number, Vec3 topLeft,
...
void Silver::Circle(const string name, int number, Vec3 center, int radius) {
...
void Silver::Line(const string name, int number, Vec3 start, Vec3 end) {
  int x1 = start.x;
  int y1 = start.y;
  int x2 = end.x;
  int y2 = end.y;
...
void Silver::Oval(const string name, int number, Vec3 center, Vec3 scale) {
...
void Silver::OvalHollow(const string name, int number, Vec3 center,
                        Vec3 scale) {
  for (int y = center.y - scale.y; y <= center.y + scale.y; ++y) {
    for (int x = center.x - scale.x; x <= center.x + scale.x; ++x) {
      int dx = x - center.x;
...
void Silver::CircleHollow(const string name, int number, Vec3 center,
                          int radius) {
  for (int y = center.y - radius; y <= center.y + radius; ++y) {
    for (int x = center.x - radius; x <= center.x + radius; ++x) {
      int dx = x - center.x;
...
void Silver::sprayRectangle(int spawns, Vec3 center, Vec3 scale, string name,
                            int number) {
  srand(static_cast<unsigned int>(time(nullptr)));
...
void Silver::sprayOval(int spawns, Vec3 center, Vec3 scale, string name,
                       int number) {
  srand(static_cast<unsigned int>(time(nullptr)));
...
void Silver::spray(int spawns, Vec3 center, int range, string name,
                   int number) {
  srand(static_cast<unsigned int>(time(nullptr)));
...
void Silver::sprayLine(int spawns, Vec3 start, Vec3 end, string name,
                       int number) {
  srand(static_cast<unsigned int>(time(nullptr)));
...
void Silver::saveChunk(Vec3 range1, Vec3 range2, const string fileName,
                       const string mode) {
  ofstream fp;
  ...
  while (getline(fp, buffer)) {
    if (buffer.rfind("📷", 0) == 0 && type == -1) {
      ...
      if (sscanf(buffer.c_str() + sizeof("➕") - 1, " \"%[^\"]\" %d %d %d %d",
        ...
        silver.placeObject(name, type, Vec3(coord1, coord2, coord3));
      }
    }
  }
}
...
void Silver::Camera::zoomCamera(Vec3 V) {
  ...
  if (silver.camera.CameraScale.x < 0)
    silver.camera.CameraScale.x = 0;
  if (silver.camera.CameraScale.y < 0)
    silver.camera.CameraScale.y = 0;
}
...
void mesh::translate3(Vec3 offset) { position += offset; }

Vec3 mesh::getPos2() { return {position.x, position.y, 0}; }

Vec3 mesh::getPos3() { return position; }
...
int Silver::UI::placeUI(const string objectName, int number, Vec3 position) {
  if (number < 0) {
    number = 0;
  }
  if (prefabrications.count(objectName) == 0) {
...
int Silver::addObject(const string objectName, Vec3 position) {
  return placeObject(objectName, silver.unoccupiedNumber(objectName), position);
}

int Silver::put(const string objectName, Vec3 position) {
  ...
  videoThread = thread([this, FPS] {
    while (isRunningCam) {
      ...
      this_thread::sleep_for(
          chrono::milliseconds(static_cast<int>(1000 / FPS)));
    }
  });
}
...
void Silver::setObjectPositionToSprite(
...
void Silver::glideObjectPositionToSprite(
...
void Silver::Particle::applyParticleComponent(int objID,
                                              const string objectName, Vec3 pos,
                                              int radius, int lifeTime,
  ...
  thread spawnThread([=]() {
    ...
    while (running) {
      ...
      Vec3 targetPos = {pos.x + offsetX, pos.y + offsetY, pos.z};
...
void Silver::Animation::applyAnimationRaw(variant<int, vector<int>> objID,
  ...
  auto animationThread = [this, ids, animation, fps, newAnimationID,
                          transition]() mutable {
    int frameIndex = 0;
...
void Silver::glideObjectRandom(const variant<int, vector<int>> &ids,
  ...
  if (holds_alternative<int>(ids)) {
    ...
    idsToUpdate.emplace_back(singleId, x_steps, y_steps,
                             Vec3{targetX, targetY, current_pos.z});
  } else {
    const vector<int> &idList = get<vector<int>>(ids);
    for (const auto &id : idList) {
...
void Silver::resetMode() { tcsetattr(STDIN_FILENO, TCSAFLUSH, &orig_termios); }
...
vector<Vec3> Silver::getRectangleHollowPoints(Vec3 topLeft, int width,
                                              int height) {
  vector<Vec3> points;
...
void Silver::applyFunction(const vector<int> &ids,
        ...
        if (mode == 'k' || mode == 'm' || mode == 'c' || mode == 'C') {
          while (true) {
            ...
            if (silver.keyboard.isKey(key)) {
              if (mode == 'c' || mode == 'C') {
                Vec3 cursorPosition = {silver.mouse.cursorPositionX,
