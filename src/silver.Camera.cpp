...
CameraManager::CameraManager() {
  ...
  activeCameras = std::vector<Camera*>();
}
...
CameraManager &CameraManager::GetInstance() {
  static CameraManager instance; // Static instance, initialized once
  return instance;
}
...
CameraManager::GetActiveCameras() {
...
void Camera::RenderFrame() {
  ...
  CameraManager &cameraManager =
      CameraManager::GetInstance(); // Use reference to Singleton



  // Prepare the buffer for rendering
  std::vector<std::vector<std::string>> buffer(
      cameraScale.y, std::vector<std::string>(cameraScale.x, "\0"));

  for (int row = 0; row < buffer.size(); row++) {
    for (int str = 0; str < buffer[row].size(); str++) {
      if (row % static_cast<int>(patternOccurrenceRate.y) == 0 &&
          str % static_cast<int>(patternOccurrenceRate.x) == 0 &&
          buffer[row][str] == "\0") {
        
        // Fill from buffer[row][str] to the length of backgroundPattern
        size_t patternLength = backgroundPattern.size();
        for (size_t i = 0; i < patternLength; ++i) {
          if (str + i < buffer[row].size()) {
            buffer[row][str + i] = backgroundPattern[i];
          }
        }
    ...
    for (int row = 0; row < buffer.size(); row++) {
      for (int str = 0; str < buffer[row].size(); str++) {
        if (row % static_cast<int>(patternOccurrenceRate.y) == 0 &&
          ...
          for (size_t i = 0; i < patternLength; ++i) {
            if (str + i < buffer[row].size()) {
              buffer[row][str + i] = outOfStagePattern[i];
            }
          }
          // Skip to the end of the pattern length
          str += patternLength - 1;
  ...
  for (const auto &entry : Workspace) {
    ...
    auto obj = *entry;
    Vector2 size = obj.GetComponent<SpriteRenderer>()->GetSize();
    Vector3 location = obj.GetComponent<Transform>()->position;
    Vector3 scale = obj.GetComponent<Transform>()->scale;

    location.x = round(location.x);
    location.y = round(location.y);
    location.z = round(location.z);
    if (location.z - scale.z >
            position.z + abs(cameraScale.z) / 2 ||
        location.z + scale.z <
            position.z -
                (abs(cameraScale.z - cameraScale.z) / 2) ||
        cameraScale.z == 0)
      continue;
    ...
    if (spriteRenderer && transform) {
      ...
      std::tuple<int, int, int, int> seek =
          obj.GetComponent<SpriteRenderer>()->GetPivotBounds();
  ...
  for (const auto &entry : Viewable) {
    ...
    Vector2 pivot = entry.GetComponent<SpriteRenderer>()->pivot;
    ...
    // printf("%lf %lf %lf %lf\n",get<0>(seek), get<1>(seek), get<2>(seek),
    // get<3>(seek));
    ...
    // printf("%lf %lf %lf %lf",r1.x,r1.y,r2.x,r2.y);
    // getchar();
    // Render the object
    for (int i = r1.x; i <= r2.x; i++) {
      for (int j = r1.y; j <= r2.y; j++) {
        // Get the cell string at the rotated position
        std::string cellStr =
            entry.GetComponent<SpriteRenderer>()->GetCellString(
                i - r1.x + pivot.x - get<0>(seek),
                j - r1.y + pivot.y - get<2>(seek));

        // Calculate the screen position
        int x = cameraScale.x / 2 + (i - position.x);
        int y = cameraScale.y / 2 + (j - position.y);

        // Update the buffer if the position is within bounds
        if (y >= 0 && y < cameraScale.y && x >= 0 &&
            x < cameraScale.x) {
          if (cellStr != " ")
            buffer[y][x] = cellStr;
        }
  ...
  for (int j = 0; j < renderedHeight; ++j) {
    ...
    if (j < cameraScale.y) {
      for (int i = 0; i < cameraScale.x; ++i) {
        string cellContent = (!(hideMouse || hideMouse) &&
                              i == mouseX && j == mouseY)
                               ? mouseIcon
                               : buffer[j][i];

        if (line.size() + cellContent.size() > availableWidth) break;
        line += cellContent;
      }
    } else {
    ...
    if (line.size() > consoleWidth - offsetX) {
      line = line.substr(0, consoleWidth - offsetX);
    }
...
void Camera::StopVideo() {
  ...
  if (isRunningCam) {
    ...
    // CameraRegion
    auto it = std::find_if(
        cameraManager.activeCameras.begin(), cameraManager.activeCameras.end(),
        [this](const Camera* camera) {
          return camera == this; // Compare the Camera pointer
        });

    if (it != cameraManager.activeCameras.end()) {
      ...
      cameraManager.activeCameras.erase(it);
    }
  }
}
...
void CleanupAndExit() {
...

  {
    ...
    for (auto camera : cameraManager.activeCameras) {
      if (camera) {
        camera->StopVideo();
      }
    }
    cameraManager.activeCameras.clear();
    Clear();
  }
...
void CameraManager::StartVideoProcessing() {
  if (isRunning) {
    ...
    while (isRunning) {
      {
        ...
        for (auto camera : activeCameras) {
          if (camera && camera->isRunningCam) {
            camera->RenderFrame(); // Process the camera
          }
        }
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(
          static_cast<int>(1000 / FPS))); // Maintain FPS
    }
  });
