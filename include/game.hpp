...
public:
    ...
    prefab(const std::string& n = "", const std::string& shp = "", bool transp = false)
...
struct zone {
  public: std::variant < std::tuple < int,
  int,
  int,
  int > ,
  ...
  zone(std::variant < std::tuple < int, int, int, int > , std::vector < std::tuple < int, int, int, int >>> shape) {}
};
...
Vec2 WorldRangeStart={-25,-25}, WorldRangeEnd={25,25};
  ...
  class UI {
    ...
    int place(std::string objectName, int number, Vec3 pos);
  };

  class Fluid {
    public: void Liquify(const std::variant < int, std::vector < int >> & IDs, double diffusionSpeed, int maximumDistance);
  ...
  class Animation {
    public: void applyAnimationRaw(std::variant < int, std::vector < int >> objID, std::vector < std::string > animation, float fps, int transition);
  ...
  void saveChunk(Vec3 range1, Vec3 range2, const std::string& fileName, const std::string& mode) ;
  class Drawing {
    public: void draw(Vec3 pos, std::string c);
  ...
  zone buildZone(std::variant < std::tuple < int, int, int, int > , std::vector < std::tuple < int, int, int, int >>> shape);
  void setObjectPosition(const std::variant < int, std::vector < int >> objectID, Vec3 pos);
  void glideObjectPositionToSprite(const std::variant < int, std::vector < int >> objectIDs, int spriteID, float speed);
  void setObjectPositionToSprite(const std::variant < int, std::vector < int >> objectIDs, int spriteID);
  void sprayRectangle(int spawns, Vec3 center, Vec3 scale, std::string name, int number);
  void sprayOval(int spawns, Vec3 center, Vec3 scale, std::string name, int number);
  void spray(int spawns, Vec3 center, int range, std::string name, int number);
  bool isAlive(int obj);
  void sprayLine(int spawns, Vec3 start, Vec3 end, std::string name, int number);
  int place(std::string objectName, int number, Vec3 pos);


  void createPrefab(const std::string name, const std::string shape);

  void moveObjectXY(const std::variant < int, std::vector < int >> objectID, Vec3 pos);
  void moveObjectX(const std::variant < int, std::vector < int >> objectID, int x_offset);
  void moveObjectY(const std::variant < int, std::vector < int >> objectID, int y_offset);
  ...
  void Rectangle(std::string name, int number, Vec3 topLeft, int width, int height);

  void RectangleHollow(std::string name, int number, Vec3 topLeft, int width, int height);

  void CircleHollow(std::string name, int number, Vec3 center, int radius);
  void Circle(std::string name, int number, Vec3 center, int radius);
  void debug(const std::string & message,
    const std::string & mode);
  void Line(std::string name, int number, Vec3 start, Vec3 end);

  void Oval(std::string name, int number, Vec3 center, Vec3 scale);
  int getRandom(int min, int max);
  void OvalHollow(std::string name, int number, Vec3 center, Vec3 scale);
  ...
  void setObjectXY(const std::variant < int, std::vector < int >> objectID, Vec3 pos);

  void glideObjectRandom(const std::variant < int, std::vector < int >> & ids,
    const std::pair < int, int > & xRange,
      const std::pair < int, int > & yRange, float speed);
  void moveObjectPosition(const std::variant < int, std::vector < int >> objectID, Vec3 pos);
  ...
  std::vector < Vec3 > getRectangleHollowPoints(Vec3 topLeft, int width, int height);
  ...
  void glideObjectX(const std::variant < int, std::vector < int >> & ids, int x_offset, float speed, ...);
  void glideObjectY(const std::variant < int, std::vector < int >> & ids, int y_offset, float speed, ...);

  void setWorldBounds(Vec2 start, Vec2 end);
  std::vector < int > seekTag(const std::string & tag);
  void applyTag(const std::vector < int > & ids,
  ...
  void walk(std::string name, std::variant < std::vector < int > , int > number, int steps, int direction);
  class Mouse {
  ...
  class Camera {
    ...
    void cell(int c);
    void configCameraException(std::string n);
  ...
  int put(std::string objectName, Vec3 pos);
  int unoccupied(std::string objectName);
  ...
  class Threading {
    ...
    std::map < std::string,
    std::shared_ptr < ThreadInfo >> threads;

    std::mutex thread_mutex;
    std::vector < std::thread > activeThreads;

    public: void tRun(std::function < void() > func);
  ...
  class Timer {
    ...
    float getDeltaTime(std::string name) const;
    ...
    long long getDuration(std::string name) const;
  ...
  std::vector < int > getLocationAxis(const std::variant < int, std::vector < int >> objectID, char axis);
  std::vector < int > getLocationX(const std::variant < int, std::vector < int >> objectID);
  std::vector < int > getLocationY(const std::variant < int, std::vector < int >> objectID);
  std::vector < int > getLocationZ(const std::variant < int, std::vector < int >> objectID);

 
  void hold();
  std::vector < int > all();
  std::vector < int > findObjects(std::string name, std::variant < std::vector < int > , int > number);
  Camera camera;
  Keyboard keyboard;
  Timer timer;
  Threading threading;
